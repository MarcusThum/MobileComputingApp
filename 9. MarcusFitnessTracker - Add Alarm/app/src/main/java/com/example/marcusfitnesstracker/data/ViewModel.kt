...
class SleepPatternViewModel(private val dao: SleepScheduleDao) : ViewModel() {
    ...
    fun addSchedule(bedTime: String, wakeUpTime: String) {
        viewModelScope.launch {
            dao.insert(SleepScheduleEntity(bedTime = bedTime, wakeUpTime = wakeUpTime))
        }
    }

    fun deleteSchedule(schedule: SleepScheduleEntity) {
        viewModelScope.launch {
            dao.delete(schedule)
        }
    }
